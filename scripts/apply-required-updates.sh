...
docker compose exec -T mysql \
    mysql -uroot -p"$LEGION_DB_ROOT_PASSWORD" \
    < "$repo_root/database/83-remove-broken-syndicate-thief-sleep.sql"

docker compose exec -T mysql \
    mysql -uroot -p"$LEGION_DB_ROOT_PASSWORD" \
    < "$repo_root/database/84-restore-dormant-creature-waypoints.sql"

docker compose exec -T mysql \
    mysql -uroot -p"$LEGION_DB_ROOT_PASSWORD" \
    < "$repo_root/database/85-add-safe-outdoor-ambient-wander.sql"

docker compose exec -T mysql \
    mysql -uroot -p"$LEGION_DB_ROOT_PASSWORD" \
    < "$repo_root/database/86-restore-vilebranch-combat-ai.sql"

docker compose exec -T mysql \
    mysql -uroot -p"$LEGION_DB_ROOT_PASSWORD" \
    < "$repo_root/database/87-restore-rocket-rescue-vehicle-smartai.sql"

docker compose exec -T mysql \
    mysql -uroot -p"$LEGION_DB_ROOT_PASSWORD" \
    < "$repo_root/database/88-fix-olrun-deaths-challenge-phase.sql"

docker compose exec -T mysql \
    mysql -uroot -p"$LEGION_DB_ROOT_PASSWORD" \
    < "$repo_root/database/89-fix-lights-point-defender-evade.sql"

docker compose exec -T mysql \
    mysql -uroot -p"$LEGION_DB_ROOT_PASSWORD" \
    < "$repo_root/database/90-fix-orbaz-acherus-gate-recovery.sql"

docker compose exec -T mysql \
    mysql -uroot -p"$LEGION_DB_ROOT_PASSWORD" \
    < "$repo_root/database/91-fix-ebon-hold-battle-phase.sql"

docker compose exec -T mysql \
    mysql -uroot -p"$LEGION_DB_ROOT_PASSWORD" \
    < "$repo_root/database/92-add-account-transmog-unlock-audit.sql"
...
docker compose exec -T mysql \
    mysql -uroot -p"$LEGION_DB_ROOT_PASSWORD" \
    < "$repo_root/database/98-restore-flames-from-above-camp.sql"
...
docker compose exec -T mysql \
    mysql -uroot -p"$LEGION_DB_ROOT_PASSWORD" \
    < "$repo_root/database/105-restore-perfecting-your-howl-credit.sql"
